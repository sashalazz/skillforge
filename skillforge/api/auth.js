...
export default async function handler(req, res) {
  ...
  if (action === "check_daily_limit") {
    ...
    // Admin has no limit
    if (decoded.isAdmin) return res.status(200).json({ success: true, allowed: true, used: count || 0, limit: dailyLimit, isAdmin: true });
